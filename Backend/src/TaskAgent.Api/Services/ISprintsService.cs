...
{
    Task<IEnumerable<SprintDto>> GetAllAsync(int currentUserId, int? projectId, CancellationToken ct = default);
    Task<SprintDto?> GetByIdAsync(int id, int currentUserId, CancellationToken ct = default);
