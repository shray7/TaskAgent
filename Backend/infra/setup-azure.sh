...
for APP in "$APP_NAME" "${APP_NAME}-staging"; do
  ...
  ENV_VARS=(
    ...
    "FileStorage__UseAzure=true"
    "FileStorage__AzureContainerName=$BLOB_CONTAINER_NAME"
    "ConnectionStrings__AzureStorage=secretref:azure-storage-connection-string"
  )
